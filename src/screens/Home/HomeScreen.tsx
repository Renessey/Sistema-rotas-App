...
export default function HomeScreen({ navigation }: Props) {
  ...
  const checkConnectivity = useCallback(async () => {
    const tiles = await RoutingService.tilesReady();
    ...
    >
      ...
      {accuracy !== null && (
        <View style={[gps.badge, { backgroundColor: dotColor + '22' }]}>
          <Text style={[gps.badgeText, { color: dotColor }]}>
...
const createGpsStyles = (colors: any) =>
  StyleSheet.create({
    card: {
      flexDirection: 'row',
      alignItems: 'center',
...
function StatTile({
  ...
  return (
    <View style={[statsGrid.tile, { borderTopColor: color }]}>
      ...
      <Text style={statsGrid.label} numberOfLines={1}>
        {label}
      </Text>
    </View>
  );
...
const createStatsGridStyles = (colors: any) =>
  StyleSheet.create({
...
function ProgressCard({ completed, total, progress }: ProgressCardProps) {
  ...
  return (
    <View style={prog.card}>
      ...
      <Text style={prog.sub}>
        {completed} de {total} entregas concluídas
      </Text>
    </View>
  );
}

const createProgStyles = (colors: any) =>
  StyleSheet.create({
    card: {
      backgroundColor: colors.surface,
      borderRadius: radius.md,
...
function ConnectivityRow({ offlineOk }: ConnectivityRowProps) {
  const { colors } = useTheme();
  ...
  return (
    <View style={[conn.row, { borderColor: status.color + '44' }]}>
      <Zap size={16} color={status.color} />
...
const createConnStyles = (colors: any) =>
  StyleSheet.create({
    row: {
      flexDirection: 'row',
      alignItems: 'center',
    ...
    >
      <View style={ds.leftContent}>
        <View style={[ds.iconBox, disabled && ds.iconBoxDisabled]}>
          <Trash2 size={18} color={disabled ? colors.textDisabled : colors.danger} />
        </View>
        ...
        <View style={dm.sheet}>
          ...
          {stats.total > 0 && (
            <View style={dm.statsBox}>
              <ModalStatTile label="Total" count={stats.total} color={colors.primary} Icon={Package} />
              <ModalStatTile label="Entregues" count={stats.completed} color={colors.success} Icon={CheckCircle2} />
              <ModalStatTile label="Pendentes" count={stats.pending} color={colors.warning} Icon={Clock} />
              <ModalStatTile label="Falhas" count={stats.failed} color={colors.danger} Icon={AlertTriangle} />
            </View>
          )}
...
const createDeleteStyles = (colors: any) =>
  StyleSheet.create({
    btn: {
      flexDirection: 'row',
      alignItems: 'center',
...
function ActionBtn({
  ...
  const iconColor = prominent ? '#FFFFFF' : disabled ? colors.textDisabled : color;
  ...
  return (
    <Pressable
      style={({ pressed }) => [
        ...containerStyle,
        !disabled && pressed ? ab.pressed : {},
      ]}
      onPress={onPress}
      disabled={disabled}
    >
      <Icon size={prominent ? 24 : 20} color={iconColor} />
      <View style={ab.textBlock}>
        <Text style={[ab.label, { color: textColor }]} numberOfLines={1}>
          {label}
        </Text>
        {sublabel ? (
          <Text style={[ab.sublabel, { color: subColor }]} numberOfLines={1}>
            {sublabel}
          </Text>
        ) : null}
      </View>
...
const createAbStyles = (colors: any) =>
  StyleSheet.create({
    btn: {
      flexDirection: 'row',
      alignItems: 'center',
...
const createQaStyles = (colors: any) =>
  StyleSheet.create({
    root: { gap: CARD_GAP },
    sectionLabel: {
      ...typography.label,
